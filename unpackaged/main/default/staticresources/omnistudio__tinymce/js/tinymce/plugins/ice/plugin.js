    ...
    tinymce.create('tinymce.plugins.IcePlugin', {
        ...
        init: function(ed, url) {
            ...
            ed.on('keydown', function(e) {
                ...
                if ((e.keyCode == 46 || e.keyCode == 8) && self.isTracking && ice.dom.browser().type != 'mozilla') {
                    e.preventDefault();
                }
            });
            ...
            ed.on('init', function(e) {
                ...
                ed.serializer.addRules(self.insertTag + '[id|class|title|style|' + self.changeIdAttribute + '|' + self.userIdAttribute + '|' + self.userNameAttribute + '|' + self.timeAttribute + ']');
                ed.serializer.addRules(self.deleteTag + '[id|class|title|style|' + self.changeIdAttribute + '|' + self.userIdAttribute + '|' + self.userNameAttribute + '|' + self.timeAttribute + ']');
                ...
                */
                ...
                var startIce = function() {
                    var args = {};
                    args.skip_focus = true;
                    if (!self.manualInit) {
                        ed.execCommand('initializeice', false, null, args);
                    }
                };
            ...
            ed.addCommand('initializeice', function(editor) {
                ...
                tinymce.DOM.win.setTimeout(function() {
                    ...
                    ed.on('keyup', function(e) {
                        console.log('In ice keyup');
                        return changeEditor.handleEvent(e);
                    });

                    ed.on('keydown', function(e) {
                        e.stopPropagation();
                        if (e.target.tagName === 'P' && (e.target.innerHTML || e.target.innerText) === '&nbsp;') {
                            e.target.innerHTML = '';
                        }
                        console.log('In ice keydown: "' + String.fromCharCode(e.which) + '", ' + '"' + e.which + '"');
                        return changeEditor.handleEvent(e);
                    });

                    ed.on('keypress', function(e) {
                        // console.log('In ice keypress: ' + String.fromCharCode(e.which));
                        return changeEditor.handleEvent(e);
                    });
            ...
            ed.addButton('iceacceptall', {
                title: 'Accept All Redlines',
                image: url + '/img/accept_all.png',
                cmd: 'iceacceptall',
            ...
            ed.addButton('icerejectall', {
                title: 'Reject All Redlines',
                image: url + '/img/reject_all.png',
                cmd: 'icerejectall',
                onPostRender: function() { //assigns button
                    var self = this;
            ...
            ed.addButton('ice_toggleshowchanges', {
                title: 'Show or Hide Redlines',
                image: url + '/img/show_or_hide.png',
                onclick: function() {
            ...
            function cleanup() {
                ...
                var isIE = /rv:/i.test(window.navigator.userAgent);
                ed.dom.remove(empty);
                ...
                tinymce.each(ed.dom.select('br'), function(br, i) {
                    ...
                    if (p && (p.innerText || p.textContent) === '' && !br.getAttribute('data-mce-bogus')) {
                        ed.dom.remove(br);
                    }
                });
                tinymce.each(ed.dom.select('p'), function(p) {
                    // console.log(p, p.textContent, p.textContent.charCodeAt(0));
                    if (isIE && p && (p.innerText.charCodeAt(0) === 32 || p.textContent.charCodeAt(0) === 160)) {
                        p.innerHTML = '';
                    }
                });
            }
